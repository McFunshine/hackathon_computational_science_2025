...
# Author: [Your Name]
# Purpose: Solves the 1D Shallow Water Equations with multiple bed profile options, including reading from a CSV file for the Thames river.
# All output filenames are prefixed with the selected bed profile for clarity.
...
zb_rhine = -(
    15 .+ 4 .* exp.(-((x .- 2.5).^2) ./ 0.25)
        .+ 2 .* exp.(-((x .- 1.5).^2) ./ 0.05)
        .+ 1.5 .* exp.(-((x .- 3.7).^2) ./ 0.08)
)

# === Read Thames bed profile from CSV (if available) ===
function read_thames_profile(nx, D)
    try
        zb_thames_file = readdlm("thames_bed_profile.csv")
        zb_thames = vec(zb_thames_file)
        if length(zb_thames) != nx
            # Interpolate or resample to match grid
            @info "Resampling Thames profile from $(length(zb_thames)) to $nx points."
            oldx = range(0, 1, length=length(zb_thames))
            newx = range(0, 1, length=nx)
            itp = interpolate((oldx,), zb_thames, Gridded(Linear()))
            zb_thames = itp.(newx)
        end
        return zb_thames
    catch e
        @warn "Could not load 'thames_bed_profile.csv', using flat bed instead. Error: $e"
        return -D * ones(nx)
    end
end

zb_thames = read_thames_profile(nx, D)

# --- CONFIG: Choose bed profile here ---
bed_profile = "thames"   # options: "flat", "wavy", "realistic", "rhine", "thames"
zb = bed_profile == "wavy"      ? zb_wavy :
     bed_profile == "realistic" ? zb_realistic :
     bed_profile == "rhine"     ? zb_rhine :
     bed_profile == "thames"    ? zb_thames :
     zb_flat
...
plt1 = plot(
    x, h_init, label="Initial h(x)", xlabel="x [m]", ylabel="Water Depth [m]",
...
plt2 = plot(
    x, q_init, label="Initial q(x)", xlabel="x [m]", ylabel="Discharge [m²/s]",
